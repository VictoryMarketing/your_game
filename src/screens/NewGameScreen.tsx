...
const WEEKLY_CHALLENGE_PREMISES: Record<string, { title: string; genre: string; atmosphere: string; prompt: string }> = {
  default: {
    title: "Тайна недели",
    genre: "Мистический детектив",
    atmosphere: "Загадочная",
    prompt: "Общий старт недели: сохрани исходный мир одинаковым для участников, но персонализируй героя и последствия решений.",
  },
};
...
function initialSettings(): StartSettings {
  ...
  try {
    serverChallenge = JSON.parse(localStorage.getItem("yougame_challenge_settings") || "{}") as Partial<StartSettings>;
  } catch {
    serverChallenge = {};
  }
  return {
    ...
    <section className="screen-stack">
      ...
      {settings.challenge_seed && (
        <section className="panel weekly-challenge-active">
          <span className="eyebrow">Общее испытание · {settings.challenge_seed}</span>
          <h2>Один мир, разные решения</h2>
          <p>Первая ситуация совпадёт с историей других участников недели. Герой, выборы и итог останутся твоими.</p>
        </section>
      )}
